...
class DeformableTransformer(nn.Module):
    def __init__(self, d_model=256, nhead=8,
                 ...
                 num_feature_levels=4, enc_n_points=4, dec_n_points=4, two_stage=False, two_stage_num_proposals=300,sigmoid_attn=False,extra_track_attn=False):
        super().__init__()
        ...
        # Decoder
        decoder_layer = CustomMambaDecoderLayer(d_model=d_model, n_levels=num_feature_levels, n_points=dec_n_points, ff_dim=dim_feedforward, dropout=dropout,extra_track_attn=extra_track_attn)
...
class DeformableTransformerEncoder(nn.Module):
    def __init__(self, encoder_layer, num_layers=3):   # 默认 3 层
        super().__init__()
...
class DeformConv1d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, groups=1, bias=True):
        ...
        self.offset_conv = nn.Conv1d(in_channels, kernel_size * groups, kernel_size, stride, padding, groups=groups, bias=True)
...
class textblock(nn.Module):  # Renamed to TextBlock as per description
    ...
    def forward(self, x, tracking_query: torch.Tensor, memory_prev: Optional[torch.Tensor] = None):
        ...
        f_primary = self.cross_attn(tracking_query, y) + tracking_query
        ...
        if memory_prev is not None:
            ...
            concat = torch.cat([f_primary.mean(dim=1), memory_prev.mean(dim=1)], dim=-1)  # [F_primary; M_t-1], using mean for simplicity
...
class CustomMambaDecoder(nn.Module):
    ...
    def forward(self, tgt, reference_points, src, spatial_shapes, level_start_index, valid_ratios,  query_pos=None, src_padding_mask=None):
        output = tgt
        ...
        for lid, layer in enumerate(self.layers):
            ...
            output = layer(output, query_pos, reference_points_input, src, spatial_shapes, level_start_index, src_padding_mask)
