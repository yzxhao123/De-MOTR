...
from .deformable_transformer_en import build_deforamble_transformer
...
from .adapter_be import BackboneEncoderAdaptor
...
class MOTR(nn.Module):
    def __init__(self, backbone, transformer, num_classes, num_queries, num_feature_levels, criterion, track_embed,
                 aux_loss=True, with_box_refine=False, two_stage=False, memory_bank=None, use_checkpoint=False,device='cuda'):
        ...
        self.text_emb_model = DefaultTextEmb(
            model_dir="/root/autodl-tmp/data1/Bert",
            text="The persons present in each video frame, used to assist tracking.",
            device="cuda"  # 或者 torch.device('cuda') / 'cpu' 根据你环境
        )

        # 将 text feature 移到 GPU
        if hasattr(self.text_emb_model, "text_features"):
            self.text_emb_model.text_features = self.text_emb_model.text_features.to("cuda")

        self.adaptor = BackboneEncoderAdaptor(
            backbone=self.backbone,
            encoder=self.transformer.encoder,
            embed_dim=256,
            fusion_scale=0.1,  # 可以保持原来的融合强度
            topk_ratio=0.2  # conv3稀疏采样比例，比如保留10%的点
        ).to(device)
    ...
    def _forward_single_image(self, samples, track_instances: Instances):
        ...
        text_emb = self.text_emb_model.get()  # (1,1,C)
        # # #
        # # # # 3. 稀疏+文本融合 adaptor
        features, pos, fused_conv3, density_map,fusion_info = self.adaptor(features, text_emb)

        #features, pos,fused_conv3 = self.adaptor(features, text_emb)
        #
        #features, pos = self.adaptor(features, text_emb)
        ...
        # for lvl, h in enumerate(hs):
        #     print(f"hs[{lvl}].shape: {h.shape}")
        ...
        out['hs'] = hs[-1]
        out['decoder_features'] = hs[-1]
        out['density_map'] = density_map
        out['fusion_info']=fusion_info
        return out
    ...
    def _post_process_single_image(self, frame_res, track_instances, is_last):
        ...
        if self.training:
            ...
            # print("TRAIN pred_logits sample:", frame_res['pred_logits'][0][:5])
            # print("TRAIN pred_boxes sample:", frame_res['pred_boxes'][0][:5])
    ...
    def inference_single_image(self, img, ori_img_size, track_instances=None):
        ...
        density_map = res.get('density_map', None)
        fusion_info = res.get('fusion_info', None)
        res = self._post_process_single_image(res, track_instances, False)

        track_instances = res['track_instances']
        track_instances = self.post_process(track_instances, ori_img_size)
        ret = {'track_instances': track_instances,'density_map': density_map}
        if 'decoder_features' in res:
            ret['decoder_features'] = res['decoder_features']
        if 'ref_pts' in res:
            ref_pts = res['ref_pts']
