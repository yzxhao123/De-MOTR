...
class Detector(object):
    def __init__(self, args):
        ...
        # 注册钩子到 layer3
        self.layer3_hook = self.model.backbone[0].body.layer3.register_forward_hook(hook_fn)
        ...
        self.save_fusion_root = os.path.join(self.save_root, 'fusion_analysis')
        Path(self.save_fusion_root).mkdir(parents=True, exist_ok=True)
        # 在现有代码之后添加
        self.save_density_root = os.path.join(self.save_root, 'density_maps')
        Path(self.save_density_root).mkdir(parents=True, exist_ok=True)
    ...
    def visualize_density_map(self, density_map, frame_id, original_img):
        """可视化密度图热力图 - 增强版"""
        # 处理密度图
        density = density_map.squeeze().cpu().numpy()

        # 增强对比度
        density = np.power(density, 0.3)  # gamma校正
        ...
        resized_density = cv2.resize(density, (original_img.shape[1], original_img.shape[0]))
        plt.imshow(resized_density, cmap='jet', alpha=0.6)
        plt.axis('off')
        plt.tight_layout()

        # 保存图像
        density_path = os.path.join(self.save_density_root, f'{frame_id:06d}_density.jpg')
        plt.savefig(density_path, bbox_inches='tight', pad_inches=0, dpi=150)
        plt.close()

    # 替换原有的 visualize_layer3_features 方法
    def visualize_layer3_features(self, frame_id, original_img):
        """可视化 backbone layer3 的特征图 - 以热力图形式"""
        if self.layer3_features is not None:
            # layer3_features shape: [B, C, H, W]
            feat = self.layer3_features[0]  # 取第一个batch

            # 对所有通道取平均得到整体激活强度
            feature_map = torch.mean(feat, dim=0).detach().cpu().numpy()

            # 归一化特征图
            feature_map = (feature_map - np.min(feature_map)) / (
                    np.max(feature_map) - np.min(feature_map) + 1e-8)
            ...
            # 调整特征图大小以匹配原图
            resized_feature = cv2.resize(feature_map, (original_img.shape[1], original_img.shape[0]))
            plt.imshow(resized_feature, cmap='jet', alpha=0.6)
            plt.axis('off')
            plt.tight_layout()

            # 保存图像
            layer3_path = os.path.join(self.save_fusion_root, f'layer3_frame_{frame_id:06d}_heatmap.jpg')
            plt.savefig(layer3_path, bbox_inches='tight', pad_inches=0)
            plt.close()
    ...
    def run(self, prob_threshold=0.7, area_threshold=100, vis=True, dump=True):
        ...
        for _, cur_img, ori_img in tqdm(self.dataloader):
            ...
            # 可视化密度图（如果存在）
            if 'density_map' in res and res['density_map'] is not None:
                self.visualize_density_map(res['density_map'], fid, ori_img)

            if vis and self.layer3_features is not None:
                self.visualize_layer3_features(fid, ori_img)
