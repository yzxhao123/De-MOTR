...
class Detector(object):
        ...
        def decoder_hook(module, input, output):
            try:
                ...
                if isinstance(output, tuple):
                    self.decoder_features = output[0] if len(output) > 0 else None
                else:
                    self.decoder_features = output

                # 获取注意力权重
                if hasattr(module, 'cross_attn') and hasattr(module.cross_attn, 'last_attention_weights'):
                    self.cross_attention_weights = module.cross_attn.last_attention_weights

                # 获取采样位置
                if hasattr(module, 'cross_attn') and hasattr(module.cross_attn, 'last_sampling_locations'):
                    self.sampling_locations = module.cross_attn.last_sampling_locations



            except Exception as e:
                print(f"Error in decoder hook: {e}")
                self.decoder_features = None

        # 注册到decoder的最后一层
        try:
            last_layer = self.model.transformer.decoder.layers[-1]
            last_layer.register_forward_hook(decoder_hook)
            print(">> Registered decoder output hook successfully")
        except Exception as e:
            print(f">> Failed to register decoder output hook: {e}")
    ...
    def visualize_deformable_attention(self, original_img, ref_pts):
        """
        可视化deformable attention - 使用热力图而不是散点
        """
        if self.cross_attention_weights is None or self.sampling_locations is None:
            return None

        try:
            # 获取数据
            attention_weights = self.cross_attention_weights.detach().cpu().numpy()
            sampling_locations = self.sampling_locations.detach().cpu().numpy()

            height, width = original_img.shape[:2]
            img_with_heatmap = original_img.copy()

            # 处理批次维度（取第0个）
            attn_weights = attention_weights[0]  # [num_queries, num_heads, num_levels, num_points]
            sample_locs = sampling_locations[0]  # [num_queries, num_heads, num_levels, num_points, 2]
            ...
            # 2. 累加注意力权重到热力图
            # ---------------------------
            for query_idx in range(attn_weights.shape[0]):
                for head_idx in range(attn_weights.shape[1]):
                    for level_idx in range(attn_weights.shape[2]):
                        for point_idx in range(attn_weights.shape[3]):

                            weight = attn_weights[query_idx, head_idx, level_idx, point_idx]

                            norm_x = sample_locs[query_idx, head_idx, level_idx, point_idx, 0]
                            norm_y = sample_locs[query_idx, head_idx, level_idx, point_idx, 1]

                            pixel_x = int(norm_x * width)
                            pixel_y = int(norm_y * height)

                            if 0 <= pixel_x < width and 0 <= pixel_y < height:
                                heatmap[pixel_y, pixel_x] += float(weight)
            ...
            # 5. 与原图叠加
            # ---------------------------
            overlay = cv2.addWeighted(img_with_heatmap, 0.4, heatmap_color, 0.6, 0)
            ...
            if hasattr(self, 'reference_points'):
                for i in range(ref_pts.shape[0]):
                    ...
                    if 0 <= px < width and 0 <= py < height:
                        cv2.line(overlay, (px - 5, py), (px + 5, py), (0, 255, 0), 2)
                        cv2.line(overlay, (px, py - 5), (px, py + 5), (0, 255, 0), 2)
    ...
    def run(self, prob_threshold=0.7, area_threshold=100, vis=True, dump=True):
        ...
        for _, cur_img, ori_img in tqdm(self.dataloader):
            ...
            if vis:
                ...
                # 调试输出注意力权重信息
                if self.cross_attention_weights is not None:
                    print(f"Frame {fid}: Cross attention weights captured, shape: {self.cross_attention_weights.shape}")
                else:
                    print(f"Frame {fid}: No cross attention weights captured")

                # 使用新的可视化方法
                decoder_viz = self.visualize_deformable_attention(ori_img, ref_pts)
                if decoder_viz is not None:
                    decoder_path = os.path.join(self.save_img_root, '{:06d}_attention.jpg'.format(fid))
                    cv2.imwrite(decoder_path, decoder_viz)
