...
class Detector(object):
    def __init__(self, args, model=None, seq_num=2):
        ...
        self.track_history = {}  # 用于跟踪每个ID的历史
        self.frame_id_switches = {}  # 存储每帧的ID切换数
    ...
    def eval_seq(self):
        data_root = os.path.join(self.args.mot_path, 'MOT17/images/test')
    ...
    def detect(self, prob_threshold=0.7, area_threshold=100, vis=False):
        ...
        for i in tqdm(range(0, self.img_len)):
            ...
            if len(dt_instances) > 0:
                current_ids = set(dt_instances.obj_idxes.numpy())
            ...
            # 这里使用简化的ID Switch计算
            frame_id_switches = len(new_ids)
            self.frame_id_switches[i + 1] = frame_id_switches
            id_switch_count += frame_id_switches
